...
fileDropDiv.on("drop", function(e){
	...
	//단일 파일 데이터만을 처리하기 떄문 첫번째 파일만 저장
	var file = files[0];
	//formFata 객체 생성, 파일 데이터 저장
...
function uploadFile(formData){
	$.ajax({
		...
		data:formData,
		dataType:"text",
		// processFata : 데이터를 일반적인 query string으로 변환처리할 것인지 결정
		...
		success : function(data){
			printFiles(data); // 첨부파일 출력 매서드 출력
			$(".noAttach").remove();
		}
	});
}
...
function printFiles(data){
	...
	// Handlebars 파일 템플잇에 파일 정보들을 바인딩하고 HTML 생성
...
//게시물 입력/수정 submit 처리시에 첨부파일 정보도 함께 처리
function filesSubmit(that){
	var str = "";
	$(".uploadedFileList .delBtn").each(function(index){
		str += "<input type='hidden' name='files["+index+"]' values= '" +$(this).attr("href") +"'>"
	});
	that.append(str);
	that.get(0).submit();
}
...
function getFileInfo(fullName){
	var originalFileName; //화면 출력할 파일명
	...
	if(checkImageType(fullName)){
		imgSrc = "/board/file/display?fileName="+fullName; //썸네일 이미지 체크
		uuidFileName = fullName.substr(14);
		var originaliImg = fullName.substr(0,12) + fullName.substr(14);
		//원본이미지 요청 링크
		originalFileUrl = "/board/file/display?fileName="+originalImg;
	} else {
		imgSrc = "/resources/img/style-switcher.png"
